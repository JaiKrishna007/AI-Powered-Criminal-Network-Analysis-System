...
export class GraphClient {
  static async fetchD4(endpoint: string, context: AuthContext, payload: any, timeoutMs: number = 10000, schema?: z.ZodTypeAny) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.error(`D4 Error [${response.status}]: ${text}`);
  ...
  static async syncEntity(context: AuthContext, entity: any) {
    return await this.fetchD4('/sync/entity', context, entity, 10000);
  }

  static async syncRelationship(context: AuthContext, relationship: any) {
    return await this.fetchD4('/sync/relationship', context, relationship, 10000);
  }

  static async syncEvidence(context: AuthContext, evidence: any) {
    return await this.fetchD4('/sync/evidence', context, evidence, 10000);
  }
}
