...
export default function RelationshipDrawer({
  ...
  const sourceNode = edgeData ? allEntities.find((e) => e.id === (edgeData as any).source) : null;
  const targetNode = edgeData ? allEntities.find((e) => e.id === (edgeData as any).target) : null;
        ...
        {loading ? (
          ...
          <div className="space-y-5">
            ...
            <div>
              ...
              <h2 className="text-base font-bold text-slate-800 mt-2">{(nodeData as any).name}</h2>
              {(nodeData as any).aliases && (nodeData as any).aliases.length > 0 && (
                <p className="text-xs text-slate-500 mt-1">
                  Aliases: <span className="text-slate-700 italic">{(nodeData as any).aliases.join(', ')}</span>
                </p>
              )}
            </div>
            ...
            <div className="p-3.5 rounded-lg bg-slate-50 border border-slate-200 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2.5 text-xs text-slate-700">
                ...
                <div>
                  ...
                  <p className="font-semibold text-slate-800">{((nodeData as any).confidence * 100).toFixed(0)}%</p>
                </div>
                {(nodeData as any).properties?.phone_value && (
                  <div className="col-span-2">
                    <p className="text-[9px] text-slate-400 font-medium">Phone Number</p>
                    <p className="font-semibold text-slate-800 font-mono">{(nodeData as any).properties?.phone_value}</p>
                  </div>
                )}
                {(nodeData as any).properties?.account_number && (
                  <div className="col-span-2">
                    <p className="text-[9px] text-slate-400 font-medium">Bank Account</p>
                    <p className="font-semibold text-slate-800 font-mono">{(nodeData as any).properties?.account_number}</p>
                  </div>
                )}
                {(nodeData as any).properties?.plate_number && (
                  <div className="col-span-2">
                    <p className="text-[9px] text-slate-400 font-medium">License Plate</p>
                    <p className="font-semibold text-slate-800 font-mono">{(nodeData as any).properties?.plate_number}</p>
                  </div>
                )}
                {(nodeData as any).properties?.address_label && (
                  <div className="col-span-2">
                    <p className="text-[9px] text-slate-400 font-medium">Physical Location</p>
                    <p className="font-semibold text-slate-800">{(nodeData as any).properties?.address_label}</p>
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="space-y-5">
            <div>
              ...
              <div className="flex items-center gap-2 mt-3 font-bold text-xs text-slate-800 bg-slate-50 p-2.5 rounded-lg border border-slate-200">
                <span className="truncate max-w-[90px]">{(sourceNode as any)?.name || (edgeData as any).source}</span>
                <ArrowRight size={14} className="text-slate-400 shrink-0" />
                <span className="truncate max-w-[90px]">{(targetNode as any)?.name || (edgeData as any).target}</span>
              </div>
            </div>
            ...
            <div className="p-3.5 rounded-lg bg-slate-50 border border-slate-200 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2 text-xs text-slate-700">
                {(edgeData as any).properties?.amount && (
                  <div className="col-span-2">
                    <p className="text-[9px] text-slate-400 font-medium">Transaction Amount</p>
                    <p className="text-base font-black text-emerald-700 mt-0.5">
                      INR {(edgeData as any).properties.amount.toLocaleString()}
                    </p>
                  </div>
                )}
                {(edgeData as any).properties?.timestamp && (
                  <div className="col-span-2">
                    <p className="text-[9px] text-slate-400 font-medium">Occurred At</p>
                    <p className="font-semibold text-slate-800 font-mono mt-0.5">
                      {new Date((edgeData as any).properties.timestamp).toUTCString()}
                    </p>
                  </div>
                )}
                {(edgeData as any).properties?.valid_from && (
                  <div>
                    <p className="text-[9px] text-slate-400 font-medium">Valid From</p>
                    <p className="font-semibold text-slate-800 font-mono mt-0.5">{(edgeData as any).properties.valid_from.split('T')[0]}</p>
                  </div>
                )}
                {(edgeData as any).properties?.valid_to && (
                  <div>
                    <p className="text-[9px] text-slate-400 font-medium">Valid To</p>
                    <p className="font-semibold text-slate-800 font-mono mt-0.5">{(edgeData as any).properties.valid_to.split('T')[0]}</p>
                  </div>
                )}
                <div>
                  <p className="text-[9px] text-slate-400 font-medium">Link Confidence</p>
                  <p className="font-semibold text-slate-800 mt-0.5">
                    {(edgeData as any).properties?.confidence ? `${((edgeData as any).properties.confidence * 100).toFixed(0)}%` : '100%'}
                  </p>
                </div>
              </div>
            </div>
