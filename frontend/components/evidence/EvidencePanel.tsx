...
import type { Evidence } from '@/lib/client-contracts/contracts';
...
export default function EvidencePanel({
  ...
  const loadEvidence = async () => {
    setLoading(true);
    try {
      const data = await d2.cases.getEvidence(caseId);
  ...
  const filteredEvidence = evidence.filter((ev) => {
    ...
    return (
      ...
      (ev.content && (ev.content as string).toLowerCase().includes(term))
    );
  });
