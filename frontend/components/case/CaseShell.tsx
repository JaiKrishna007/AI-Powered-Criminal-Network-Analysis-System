...
export default function CaseShell({
  ...
  useEffect(() => {
    async function fetchCases() {
      try {
        const data = await d2.cases.list();
        if (data && data.cases) {
          setCases(data.cases);
  ...
  const navItems = [
    ...
    { id: 'report', label: 'Reports', icon: FileSignature },
  ];
  ...
  return (
    <div className="flex h-screen bg-[#F8FAFC] text-slate-800 font-sans">
      ...
      <aside className="w-64 bg-[#0F172A] flex flex-col z-20 text-slate-350 shrink-0">
        {/* Brand Logo & Security Header */}
        <div className="p-5 border-b border-slate-800 flex flex-col gap-2 bg-[#0F172A]">
          <div className="flex items-center gap-3">
            <img src="/static/netra-logo.png" alt="NETRA Logo" className="w-8 h-8 object-contain" />
            <div>
              <h1 className="font-bold text-sm tracking-wide text-white">
                NETRA
              </h1>
              <p className="text-[7px] text-slate-400 font-bold tracking-wider uppercase">
                AI-POWERED CRIMINAL NETWORK ANALYSIS SYSTEM
              </p>
            </div>
          </div>
          {/* Subtle Clearance Status Badge */}
          <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-[#1E293B] border border-slate-800 text-slate-300 text-[10px] font-bold mt-2 uppercase tracking-wider">
            <ShieldAlert size={12} className="shrink-0 text-amber-500" />
          ...
          {loading ? (
            ...
            <div>
              <button
                ...
                className="w-full flex items-center justify-between px-3 py-2.5 rounded-lg bg-slate-900 border border-slate-800 hover:border-slate-700 transition text-left text-xs font-semibold text-slate-200"
              >
              ...
              {showCaseSelector && (
                <div className="absolute top-[68px] left-4 right-4 bg-[#0F172A] border border-slate-800 rounded-lg shadow-xl overflow-hidden z-30">
                  {cases.map((c) => (
                    <button
                      key={c.id}
                      onClick={() => {
                      ...
                      className={`w-full text-left px-4 py-3 text-xs hover:bg-slate-800 border-b border-slate-800 last:border-b-0 transition flex flex-col gap-1 ${
                        activeCaseId === c.id ? 'bg-slate-800 border-l-2 border-l-blue-500' : ''
                      }`}
                    >
        ...
        <div className="p-4 border-t border-slate-800 bg-[#090E1A] flex items-center justify-between text-xs text-white">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center font-bold text-slate-350">
              A
            </div>
            <div>
              <p className="font-semibold text-slate-200">Arash</p>
              <p className="text-[10px] text-slate-400 font-medium">Investigator</p>
            </div>
          </div>
          <div className="flex items-center gap-1 text-emerald-400 text-[10px] font-bold bg-emerald-950/20 border border-emerald-900/30 px-2 py-0.5 rounded uppercase animate-pulse">
            <UserCheck size={10} />
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden bg-[#F8FAFC]">
        {/* Top Header Panel */}
        <header className="h-16 border-b border-slate-200 bg-white flex items-center justify-between px-6 z-10 shadow-sm shrink-0">
          ...
          <div className="flex items-center gap-4 text-xs font-semibold text-slate-550">
            {activeCase && (
              <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-slate-50 border border-slate-200 text-slate-700 shadow-sm">
