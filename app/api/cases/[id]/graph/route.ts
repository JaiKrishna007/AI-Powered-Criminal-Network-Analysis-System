...
export async function GET(
  ...
  // Enforce strict connection check - fail fast if Neo4j is not connected (No Mockdata rule)
  if (!neo4jService.isConnected()) {
    return NextResponse.json({
      error: 'NEO4J_CONNECTION_FAILED',
      message: 'Neo4j database driver is not connected. Ensure NEO4J_URI, NEO4J_USER, and NEO4J_PASSWORD are set.'
    }, { status: 502 });
  }
